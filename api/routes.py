...
# Example: http://127.0.0.1:5000/reports/count_by_crime_code?start_date=2024-01-01&end_date=2024-01-31
...
# Example: http://127.0.0.1:5000/reports/count_by_day?start_date=2024-01-01&end_date=2024-01-31&crm_cd=330
...
def count_by_day():
    ...
    try:
        ...
        result = db.crime_reports.aggregate([
            {"$match": {"date_occ": {"$gte": start_date, "$lte": end_date}, "crm_cd_list.code": int(crm_cd)}},
            {"$group": {"_id": {"date": "$date_occ"}, "count": {"$sum": 1}}},
            {"$sort": {"_id": 1}}
        ])
...
def top_crimes_by_area():
    ...
    try:
        pipeline = [
            ...
            {"$unwind": "$crm_cd_list"},
            {"$group": {"_id": {"area_code": "$area_code", "crime": "$crm_cd_list"}, "count": {"$sum": 1}}},
            {"$sort": {"_id.area_code": 1, "count": -1}},
            {"$group": {"_id": "$_id.area_code", "top_crimes": {"$push": {"crime": "$_id.crime", "count": "$count"}}}},
            {"$project": {"area_code": "$_id", "top_crimes": {"$slice": ["$top_crimes", 3]}, "_id": 0}}
        ]
...
def least_common_crimes():
    ...
    try:
        pipeline = [
            ...
            {"$group": {"_id": "$crm_cd_list", "count": {"$sum": 1}}},
            {"$sort": {"count": 1}},
            {"$limit": 2}
        ]
        result = list(db.crime_reports.aggregate(pipeline))
...
def weapons_by_crime_across_areas():
    ...
    try:
        pipeline = [
            ...
            {"$group": {"_id": {"crime_code": "$crm_cd_list.code", "weapon_code": "$weapon_code"}, 
                        "areas": {"$addToSet": "$area_code"}}},
            {"$project": {"crime_code": "$_id.crime_code", "weapon_code": "$_id.weapon_code",
                          "area_count": {"$size": "$areas"}}},
            {"$match": {"area_count": {"$gt": 1}, "weapon_code": {"$ne": None}}},
            {"$lookup": {"from": "weapons", "localField": "weapon_code", "foreignField": "weapon_code",
                         "as": "weapon_info"
            }},
            {"$unwind": "$weapon_info"},
            {"$project": {"_id": 0, "crime_code": 1, "weapon_code": 1, "weapon_desc": "$weapon_info.weapon_desc",
                          "area_count": 1}}
        ]
        result = list(db.crime_reports.aggregate(pipeline))
        return jsonify(result)
...
def top_50_upvoted_reports():
    ...
    try:
        pipeline = [
            {"$match": {"date_occ": {"$gte": target_date, "$lt": next_date}}},
            {"$lookup": {"from": "upvotes", "localField": "dr_no", "foreignField": "dr_no", "as": "upvotes"}},
            {"$addFields": {"upvote_count": {"$size": "$upvotes"}}},
            {"$sort": {"upvote_count": -1}},
            {"$limit": 50},
            {"$project": {"_id": 0, "dr_no": 1, "upvote_count": 1}}
        ]
        result = list(db.crime_reports.aggregate(pipeline))
        return jsonify(result)
    except Exception as e:
...
def top50_most_active_officers():
    ...
    try:
        pipeline = [
            {"$group": {"_id": {"badge_number": "$badge_number", "name": "$officer_name", 
                                "email": "$officer_email"}, "upvote_count": {"$sum": 1}}},
            {"$sort": {"upvote_count": -1}},
            {"$limit": 50},
            {"$project": {"_id": 0, "badge_number": "$_id.badge_number", "name": "$_id.name",
                          "email": "$_id.email", "upvote_count": 1}}
        ]
...
def top50_officers_by_areas():
    ...
    try:
        pipeline = [
            {"$lookup": {"from": "crime_reports", "localField": "dr_no", "foreignField": "dr_no", "as": "crime_report"}},
            {"$unwind": "$crime_report"},
            {"$group": {"_id": {"badge_number": "$badge_number", "name": "$officer_name", "email": "$officer_email"}, 
                        "areas": {"$addToSet": "$crime_report.area_code"}}},
            {"$project": {"_id": 0, "badge_number": "$_id.badge_number", "name": "$_id.name",
                          "email": "$_id.email", "area_count": {"$size": "$areas"}}},
            {"$sort": {"area_count": -1}},
            {"$limit": 50}
        ]
        result = list(db.upvotes.aggregate(pipeline))
...
def problematic_reports():
    ...
    try:
        pipeline = [
            {"$group": {"_id": {"email": "$officer_email", "dr_no": "$dr_no"}, 
                        "badge_numbers": {"$addToSet": "$badge_number"}}},
            {"$match": {"$expr": {"$gt": [{"$size": "$badge_numbers"}, 1]}}},
            {"$group": {"_id": None, "reports": {"$addToSet": "$_id.dr_no"}}},
...
def voted_areas_by_officer():
    ...
    try:
        pipeline = [
            ...
            {"$lookup": {"from": "crime_reports", "localField": "dr_no", "foreignField": "dr_no",
                         "as": "crime_report"}},
            {"$unwind": "$crime_report"},
            {"$group": {"_id": "$crime_report.area_code"}},
            {"$lookup": {"from": "areas", "localField": "_id", "foreignField": "area_code",
                         "as": "area_info"}},
            {"$unwind": "$area_info"},
            {"$project": {"_id": 0, "area_code": "$_id", "area_name": "$area_info.area_name"}}
        ]
        result = list(db.upvotes.aggregate(pipeline))
        return jsonify(result)
    except Exception as e:
